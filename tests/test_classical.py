...
class TimeDomainIndexesTestCase(unittest.TestCase):
    def test_correct_response(self):
        ...
        np.testing.assert_almost_equal(
            sorted(response.values()), sorted(expected.values()), decimal=2
        )
...
class FrequencyDomainTestCase(unittest.TestCase):
    ...
    def test_frequency_domain_with_welch_and_detrended_rri_and_interp(
        ...
        fake_rri = RRiDetrended([-1, -2, -3, -4], time=[5, 6, 7, 8], interpolated=True)
        frequency_domain(
            fake_rri,
            time=fake_rri.time,
            fs=4,
    ...
    def test_area_under_the_curve(self):
        ...
        results = _auc(
            fxx, pxx, vlf_band=(0, 0.04), lf_band=(0.04, 0.15), hf_band=(0.15, 0.4)
        )
    ...
    def test_pburg_method_being_called(self, _pburg):
        ...
        _pburg.assert_called_once_with(
            data=[1, 2, 3], NFFT=None, sampling=4.0, order=16
        )

    @mock.patch("hrv.classical._auc")
    @mock.patch("hrv.classical._interpolate_rri")
    @mock.patch("hrv.classical._calc_pburg_psd")
    def test_frequency_domain_function_using_pburg(self, _pburg_psd, _irr, _auc):
        fake_rri = [1, 2, 3, 4]
        _irr.return_value = fake_rri
        _pburg_psd.return_value = (np.array([1, 2]), np.array([3, 4]))
        frequency_domain(fake_rri, fs=4, method="ar", interp_method="cubic", order=16)
        expected_called_rri = [-0.19565217, 0.15217391, 0.17391304, -0.13043478]

        pburg_args_list = _pburg_psd.call_args_list[0][1]
        assert isinstance(pburg_args_list["rri"], RRiDetrended)
        np.testing.assert_almost_equal(
            expected_called_rri, pburg_args_list["rri"].values
        )
    ...
    def test_frequency_domain_with_pburg_and_detrend(
        ...
        frequency_domain(
            fake_rri, time=fake_time, fs=4, method="ar", order=16, detrend="linear"
        )

        _interpolate_rri.assert_called_once_with(fake_rri, fake_time, 4, "cubic")
        _poly_detrend.assert_called_once_with([1, 2, 3, 4], degree=1)
        _pburg.assert_called_once_with(
            rri=[-1, -2, -3, -4], fs=4, order=16,
        )
